...
(function () {
  ...
  // Braille spinner driver — Open Field loading is monospace characters.
  const CF_SPIN_FRAMES = ["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"];
  let cfSpinTick = 0;
  setInterval(() => {
    const els = document.getElementsByClassName("cf-spinner");
    if (!els.length) return;
    cfSpinTick = (cfSpinTick + 1) % CF_SPIN_FRAMES.length;
    for (const el of els) el.textContent = CF_SPIN_FRAMES[cfSpinTick];
  }, 90);
      ...
      if (isBatchStale) {
        ...
        banner.innerHTML = `
          <div class="cf-processing-row">
            ...
            <div class="cf-processing-body">
              <strong>Your agent is working on it</strong>
              <span class="cf-processing-meta">${n} comment${n === 1 ? "" : "s"} · submitted ${submittedAgo}</span>
            </div>
          </div>
          <details class="cf-processing-details">
